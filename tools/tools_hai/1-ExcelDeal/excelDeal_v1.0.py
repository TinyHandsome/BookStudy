...
def visualize():
    ...
    # 函数区
    def dealExcel(save_path, path1, path2, left, right, way):
        """
        处理两个Excel，返回结果
    ...
    def run():
        ...
        try:
            ...
            # dealExcel(save_file.get(), path1_file.get(), path2_file.get(), words_1, words_2, var_ratio.get())
            thread_it(dealExcel,
    ...
    e4.insert(tk.END, ' 需要匹配的关键词，用空格隔开（不写则默认两表的相同字段）')
    ...
    b4.grid(row=5, column=2, sticky='wesn')
    ...
    e5.insert(tk.END, ' 需要匹配的关键词，用空格隔开（不写则默认两表的相同字段）')
    ...
    # 单选框点击背景色
    bc = None
    r1 = tk.Radiobutton(root, text='左连接', value='left', height=ratio_height, variable=var_ratio, font=ratio_font,
                        relief=style, activebackground=bc)
    r1.grid(row=3, column=2, sticky='wesn')
    r2 = tk.Radiobutton(root, text='内连接', value='inner', height=ratio_height, variable=var_ratio, font=ratio_font,
                        relief=style, activebackground=bc)
    r2.grid(row=4, column=2, sticky='wesn')
